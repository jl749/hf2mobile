...
*What hf2mobile's QNN path buys, measured on a Galaxy S25+.* ([back to README](../README.md))
...
| Qwen2.5-3B | 8.48 | 15.63 | **22.25** | 21.77 | n/a | n/a |
| Qwen3-4B | n/a | 10.76 | **17.49** | 17.45 | n/a | n/a |

> [!IMPORTANT]
> The NPU reaches **2.4-3.4× the CPU throughput wherever both run**, and past 1.7B it is the only configuration that runs at all.
>
> **NPU W4 is the fastest configuration at every size**, which indicates that this workload is memory-bound rather than compute-bound: fewer weight bits yield proportionally more throughput.
...
- **CPU fp32 at 3B and 4B.** 12.4 GB and 16.1 GB against roughly 6.2 GB of available RAM. The 3B attempt ran for 4m11s and then rebooted the device; 4B fp32 cannot even be exported on the host (16.1 GB resident against 13 GB free, no swap).
...
Deployable bytes, meaning NPU context binaries plus the fp16 main graph, against the CPU fp32 file:
